 ...
 * governance (T-28) and memory governance (T-34): the single source of truth
 * is OMP's own settings store (`~/.omp/agent/config.yml`, surfaced through
 * the SDK `Settings` singleton) — the deck never persists a copy of these
 * values. Reads are call-time inside the SDK, so a change here applies to
 * the next request even in live sessions.
 */
